...
public sealed class ActorRepositoryStruct<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TActor, TRequest> : IActorRepositoryRunnable where TActor : IActorStruct<TRequest> where TRequest : struct
{
    ...
    public IActorRefStruct<TActor, TRequest> SpawnWithOptions(string? name, ActorRunnerOptions? options, params object[]? args)
    {
        ...
        Lazy<(ActorRunnerStruct<TActor, TRequest> runner, ActorRefStruct<TActor, TRequest> actorRef)> created = new(() => CreateInternal(name, maxInboxSize, args));
        Lazy<(ActorRunnerStruct<TActor, TRequest> runner, ActorRefStruct<TActor, TRequest> actorRef)> actor = actors.GetOrAdd(name, created);
    ...
    private (ActorRunnerStruct<TActor, TRequest>, ActorRefStruct<TActor, TRequest>) CreateInternal(string name, int? maxInboxSize, params object[]? args)
    {
    ...
    public async Task GracefulShutdownAll(TimeSpan maxWait)
    {
        ...
        foreach (KeyValuePair<string, Lazy<(ActorRunnerStruct<TActor, TRequest> runner, ActorRefStruct<TActor, TRequest> actorRef)>> kv in actors)
        {
            names.Add(kv.Key);
