...
public sealed class ActorRepository<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TActor, TRequest> : IActorRepositoryRunnable where TActor : IActor<TRequest> where TRequest : class
{
    ...
    public IActorRef<TActor, TRequest> SpawnWithOptions(string? name, ActorRunnerOptions? options, params object[]? args)
    {
        ...
        Lazy<(ActorRunner<TActor, TRequest> runner, ActorRef<TActor, TRequest> actorRef)> created = new(() => CreateInternal(name, maxInboxSize, args));
        Lazy<(ActorRunner<TActor, TRequest> runner, ActorRef<TActor, TRequest> actorRef)> actor = actors.GetOrAdd(name, created);
    ...
    private (ActorRunner<TActor, TRequest>, ActorRef<TActor, TRequest>) CreateInternal(string name, int? maxInboxSize, params object[]? args)
    {
        ActorRunner<TActor, TRequest> runner = new(actorSystem, logger, name, maxInboxSize);

        ActorRef<TActor, TRequest> actorRef = new(runner);
    ...
    public async Task GracefulShutdownAll(TimeSpan maxWait)
    {
        ...
        foreach (KeyValuePair<string, Lazy<(ActorRunner<TActor, TRequest> runner, ActorRef<TActor, TRequest> actorRef)>> kv in actors)
        {
            names.Add(kv.Key);
