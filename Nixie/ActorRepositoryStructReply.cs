...
public sealed class ActorRepositoryStruct<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TActor, TRequest, TResponse> : IActorRepositoryRunnable
    where TActor : IActorStruct<TRequest, TResponse> where TRequest : struct where TResponse : struct
{
    ...
    public IActorRefStruct<TActor, TRequest, TResponse> SpawnWithOptions(string? name, ActorRunnerOptions? options, params object[]? args)
    {
        ...
        Lazy<(ActorRunnerStruct<TActor, TRequest, TResponse> runner, ActorRefStruct<TActor, TRequest, TResponse> actorRef)> created = new(() => CreateInternal(name, maxInboxSize, isControlMessage, args));
        Lazy<(ActorRunnerStruct<TActor, TRequest, TResponse> runner, ActorRefStruct<TActor, TRequest, TResponse> actorRef)> actor = actors.GetOrAdd(name, created);
    ...
    private (ActorRunnerStruct<TActor, TRequest, TResponse>, ActorRefStruct<TActor, TRequest, TResponse>) CreateInternal(string name, int? maxInboxSize, Func<TRequest, bool>? isControlMessage, params object[]? args)
    {
        ...
        ActorRefStruct<TActor, TRequest, TResponse> actorRef = new(runner);
